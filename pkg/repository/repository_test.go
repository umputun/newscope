...
func TestSplitMigrationStatements(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			input: `CREATE INDEX idx1 ON table1(col1);
...
ON table2(col2);`,
		...
		},
		{
			...
			input: `CREATE TRIGGER IF NOT EXISTS test_trigger AFTER INSERT ON items BEGIN
    INSERT INTO items_fts(rowid, title)
    VALUES (new.id, new.title);
END;
CREATE INDEX idx1 ON table1(col1);`,
			expected: []string{
				"CREATE TRIGGER IF NOT EXISTS test_trigger AFTER INSERT ON items BEGIN\n    INSERT INTO items_fts(rowid, title)\n    VALUES (new.id, new.title);\nEND;",
				"CREATE INDEX idx1 ON table1(col1);",
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := splitMigrationStatements(tc.input)
