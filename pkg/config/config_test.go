...
func TestConfig_ValidateEdgeCases(t *testing.T) {
	...
	t.Run("score threshold too low", func(t *testing.T) {
		cfg := &Config{
			LLM: LLMConfig{
				Endpoint: "https://api.openai.com/v1",
				APIKey:   "test-key",
		...
		err := validate(cfg)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "llm.classification.score_threshold must be between 0 and 10")
	})

	t.Run("score threshold too high", func(t *testing.T) {
		cfg := &Config{
			LLM: LLMConfig{
				Endpoint: "https://api.openai.com/v1",
				APIKey:   "test-key",
	...
	t.Run("score threshold boundary values valid", func(t *testing.T) {
		cfg := &Config{
			LLM: LLMConfig{
				Endpoint: "https://api.openai.com/v1",
				APIKey:   "test-key",
				Model:    "gpt-4",
				Classification: ClassificationConfig{
					ScoreThreshold: 0.0, // boundary value
				},
			},
			Server: struct {
				Listen   string        `yaml:"listen" json:"listen" jsonschema:"default=:8080,description=HTTP server listen address"`
		...
		cfg.LLM.Classification.ScoreThreshold = 10.0 // boundary value
		err = validate(cfg)
		assert.NoError(t, err)
	})
